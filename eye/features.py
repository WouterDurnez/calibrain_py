...
class EyeFeatures:
    ...
    def load_params(self, **params):
        ...
        self.gaze_object_col = (
            params["gaze_object_col"] if "gaze_object_col" in params else "gaze_object"
        )
        self.aoi_mapping = params["aoi_mapping"] if "aoi_mapping" in params else None
    ...
    def create_transition_matrix(self):
        ...
        df = pd.DataFrame(
            self.data[self.gaze_object_col] if "AOI" not in self.data else self.data.AOI
        )
        df.columns = ["AOI"]
        ...
        df["count"] = 1
        transition_matrix = df.groupby(["AOI", "AOI_shift"]).count().unstack().fillna(0)
        transition_matrix.columns = transition_matrix.columns.get_level_values(1)
        ...
        transition_matrix.columns = [str(lvl) for lvl in transition_matrix.columns]
    ...
    def get_entropy(self, quiet: bool = True) -> float:
        ...
        A = np.append(np.transpose(M_norm) - np.identity(dim), [[1] * dim], axis=0)
        ...
        for i in range(M_norm.shape[0]):
            for j in range(M_norm.shape[1]):
                val = 0 if M_norm[i, j] == 0 else M_norm[i, j] * np.log(M_norm[i, j])
    ...
    def get_gaze_switches(self, mode: str = "absolute", to: str | list = None):
        ...
        if not to:
            self.absolute_gaze_switches = (
                self.transition_matrix.values.sum() - np.trace(self.transition_matrix)
            )
            ...
            if mode == "relative":
                self.relative_gaze_switches = (
                    self.absolute_gaze_switches / self.transition_matrix.values.sum()
                )
        ...
        # (Make sure it's not a list, but this shouldn't happen anymore.)
            ...
            setattr(self, f"absolute_gaze_switches_to_{to}", data.check.count())
