...
class EyePreprocessor:
    ...
    def load_params(self, **params):
        ...
        params.setdefault("clean_missing_data_params", True)
        params.setdefault("add_velocity_params", True)
        params.setdefault("remove_outliers_params", True)
        params.setdefault("remove_edge_artifacts_params", True)
        self.params = params
    ...
    def clean_missing_data(self, to_replace: list | float = -1, value=np.nan):
        ...
        to_replace = [to_replace] if not isinstance(to_replace, list) else to_replace
    ...
    def calculate_gaze_velocity(self, shift: int = 1):
        ...
        velocity_to_next = np.abs(
            (self.data[self.pupil_col] - self.data[self.pupil_col].shift(periods=shift))
            / (self.data[self.time_col] - self.data[self.time_col].shift(periods=shift))
        )
    ...
    def remove_edge_artifacts(
        ...
        for idx, (start, stop, _) in enumerate(gaps):
            ...
            self.data.loc[
                self.data[self.time_col].between(start, stop, inclusive="left"),
                self.pupil_col,
            ] = np.nan
    ...
    def pipeline(self, data: pd.DataFrame = None, **params):
        ...
        if "velocity" not in self.data or add_velocity_params:
            ...
            self.data["velocity"] = self.calculate_gaze_velocity(**add_velocity_params)

        # Clean data based on_col outlier criterion
        if remove_outliers_params:
            log("⚙️ Removing outliers based on MAD.", color="green")
        ...
        if remove_edge_artifacts_params:
            log("⚙️ Removing edge artifacts.", color="green")
...
if __name__ == "__main__":
    ...
    for args in (
        ...
        data["left_pupil_size_smoothed"] = data.left_pupil_size.interpolate(**args)
        fig.add_trace(
            go.Scatter(
                x=data.index,
                y=data.left_pupil_size_smoothed,
                mode="lines",
                name=f'left_pupil_size_smoothed_{args["method"]}{"_"+str(args["order"]) if "order" in args else ""}',
                opacity=0.3,
            )
        )
    fig.show()
