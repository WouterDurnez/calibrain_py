...
class EyePreprocessor(Processor):
    ...
    def load_params(self, **params):
        ...
        self.pupil_col = (
            self.params['pupil_col']
            if 'pupil_col' in params
            else 'left_pupil_size'
        )
        self.time_col = (
            self.params['time_col'] if 'time_col' in params else 'timestamp'
        )

        # If no step arguments are given: run everything with default parameters
        self.params.setdefault('clean_missing_data_params', True)
        self.params.setdefault('add_velocity_params', True)
        self.params.setdefault('remove_outliers_params', True)
        self.params.setdefault('remove_edge_artifacts_params', True)
    ...
    def pipeline(self, data: pd.DataFrame = None, **params):
        ...
        remove_edge_artifacts_params = self.params[
...
if __name__ == '__main__':
    ...
    # Try pipeline
    ep = EyePreprocessor(data=data, **eye_preprocessing_params)
    preprocessed_data = ep.pipeline()
