...
if __name__ == "__main__":
    ...
    clt_agg_subj = get_aggregated_subjective_data(database, 'clt').reset_index(drop=True)
    mrt_agg_subj = get_aggregated_subjective_data(database, 'mrt').reset_index(drop=True)
    # without physical demand in mrt
    mrt_agg_subj['nasa_without_pd'] = mrt_agg_subj[
        ['md', 'td', 'pe', 'ef', 'fl']
    ].mean(axis=1)
    # save this data for analysis in R
    clt_agg_subj.to_csv(os.path.join('/Users/jonas/Desktop/CaliBrain_preprocdata', 'clt_subj.csv'))
    mrt_agg_subj.to_csv(os.path.join('/Users/jonas/Desktop/CaliBrain_preprocdata', 'mrt_subj.csv'))
    ...
    mrt_agg_perf.to_csv(os.path.join('/Users/jonas/Desktop/CaliBrain_preprocdata', 'mrt_perf.csv'))
